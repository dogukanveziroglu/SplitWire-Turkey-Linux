...
> **WIP (Gelistirme Asamasinda)** — Bu proje aktif olarak gelistirilmektedir. Ozellikler eksik olabilir, API'ler degisebilir ve dokumantasyon guncel durumu yansitmayabilir.

Linux icin acik kaynakli bir ag gizliligi ve trafik yonetimi araci. VPN ayrik tunelleme, DNS-over-HTTPS yapilandirmasi, DPI bypass araclari ve uygulamaya ozel yonlendirme ozellikleri saglar.

## Proje Durumu

| Bilesen | Durum | Notlar |
|---------|-------|--------|
| Core (config, logger, shell) | Kararlı | Tamamen test edildi |
| WireGuard VPN + split tunnel | Uygulandı | Gercek ortam testi gerekli |
| Zapret (nfqws/tpws) | Uygulandı | DPI bypass |
| ByeDPI (ciadpi proxy) | Uygulandı | `split` preset Turk ISP'leri icin calisiyor |
| DNS yonetimi (DoH) | Uygulandı | Cloudflare/Google/Quad9 |
| Discord onarim | Uygulandı | Tespit + alternatif istemciler |
| GTK4/Libadwaita UI | Uygulandı | 5 sayfa |
| Unit testler | 371 basarili | Mock testlerde tam kapsam |
| Integration testler | WIP | Docker ortami hazir |
| E2E bypass testler | WIP | discord.com ~240ms, roblox.com ~370ms |
